...
const rightsSteps = [
  ...
  {
    id: "03",
    title: "הכשרת 'שגרירי זכויות' במחוזות",
    bullets: [
      "הסמכת נציגים מכל שבעת המחוזות כ-'שגרירי זכויות אתיות-משפטיות' שיעברו הכשרה מקצועית עם עורך הדין של הוועדה.",
      "השגרירים ישמשו ככתובת ראשונה וזמינה לכל פנייה, שאלה או עוולה שמתרחשת ברשויות ובבתי הספר במחוז שלהם.",
      "בניית רשת ביטחון ארצית שמוודאת שאף נציג או תלמיד לא עומד לבד מול המערכת."
    ]
  }
];
...
export default function Rights() {
  return (
    <section className="rightsPageContainer almoni">
      ...
      {/* 5. קריאה לפעולה */}
      <div className="eduSection rightsCallToAction" style={{ marginBottom: "60px", marginTop: "40px" }}>
        <h2 className="ctaMain">
          אף נציג ואף תלמיד לא יישאר לבד במערכה!
        </h2>
        <h3 style={{ maxWidth: "850px", margin: "0 auto 30px", fontSize: "16px", lineHeight: "1.6", color: "var(--text)" }}>
          הגנה על זכויות דורשת אומץ ציבורי, נחישות וכלים משפטיים חדים. יחד עם הדרגים המקצועיים ועורכי הדין של הוועדה, נהפוך את הוועדה האתית-משפטית לכתובת האמינה והחזקה ביותר עבורכם. נדרוש דיונים דחופים בוועדות הכנסת, נוציא מכתבי התראה לגופים מפרים ונשמור על הנוער בכל מקום בו הוא נמצא.
        </h3>
        <div className="ctaButtons">
          <Link to="/contact" className="ctaPrimaryBtn" style={{ textDecoration: "none", display: "inline-block" }}>
            דווח על הפרת זכויות
          </Link>
        </div>
      </div>
